...
export default function Page() {
  ...
  const faq = [
    {
      q: "高齢者支援体制スコアとは何ですか？",
      a: "高齢化率だけを見ていても分からない「その地域の高齢者がどれだけ支援を受けやすいか」を数値化するために、3つの実データを組み合わせました。具体的には人口あたりの医師数、高齢者あたりの老人ホーム定員数、独居高齢者の割合をそれぞれ統計的に標準化し、足し合わせています。",
    },
    {
  ...
  return (
    ...
    >
      ...
      <div style={box}>
        <h2>数字だけで安心・心配しすぎないために</h2>

        <p>
          医師数の統計は勤務地ベースなので、近隣自治体の
          病院に通っている高齢者はこのスコアに反映されません。
          「うちの町は医師が少ないから不安」と感じても、
          車で30分の隣町に総合病院があれば、実際の生活は
          スコアほど厳しくないこともあります。逆に、人口の
          少ない自治体では老人ホームが1つ増減するだけで
          スコアが大きく動くので、細かい順位の上下に一喜一憂
          する必要はありません。あくまで「その地域を知る
          ための1つの手がかり」として使ってください。
        </p>
      </div>
