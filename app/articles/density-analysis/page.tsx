...
export default function Page() {
  ...
  const funabashi = getMunicipalities().find((c) =>
    c.name.includes("船橋市")
  );
  const ichikawa = getMunicipalities().find((c) =>
    c.name.includes("市川市")
  );
  ...
  return (
    ...
    >
      ...
      <div style={box}>
        ...
        <p>
          人口密度の数字は、実際に住んでいる人の体感とズレることも
          少なくありません。「人が多い街」と「人口密度が高い街」は、
          必ずしも同じではないのです。
        </p>

        <PersonalNote>
          千葉県の船橋市と市川市には土地勘があるのですが、どちらも
          「都心に近く人が多いベッドタウン」という似た印象を
          持っていました。実際に船橋市は人口密度
          {funabashi?.populationDensity?.toLocaleString()}
          人/km²と体感どおり高い水準です。ただ意外だったのは
          市川市で、こちらは落ち着いた住宅地というイメージが
          強かったのですが、人口密度は
          {ichikawa?.populationDensity?.toLocaleString()}
          人/km²と船橋市を上回っていました。「静か＝低密度」
          とは限らないと、データを見て初めて気づいた例です。
        </PersonalNote>
      </div>
