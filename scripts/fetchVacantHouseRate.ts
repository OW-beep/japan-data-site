...
//      全国、都道府県、市区町村」(公開年月 202310)
//
// 実際にレスポンスを取得して確認した結果、区分コードの絞り込み
// および地域コードの桁数は問題なく、正しく値が取得できることを
// 確認済み。
//
// 【実行済み】既に実行済みで、data/cities.json の
// vacantHouseCount / totalHousingCount への反映は完了している。
// (人口1万5千人未満の町村は総務省統計局の公表方針により対象外
// のため、該当する行は null のままになる。これは仕様であり不具合
// ではない。) e-Stat側の統計が更新されない限り、再実行しても
// 結果は変わらない(冪等)。次回の更新は次回の住宅・土地統計調査
// (5年に1度)を待つことになる。
// ------------------------------------------------------------------
