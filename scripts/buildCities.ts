...
function safeValues(
  ...
  if (!values) {
    console.error(
      `⚠ 「${label}」のデータが空でした。このカテゴリコードは` +
        `処理をスキップします(該当フィールドは0のままになります)。`
    );
    return [];
  }
...
async function fetchStats(
  ...
  const status =
    json?.GET_STATS_DATA?.RESULT?.STATUS;

  if (status !== 0 && status !== undefined) {
    console.error(
      `⚠ e-Stat APIエラー (コード: ${code}): STATUS=${status} ` +
        `MSG=${json?.GET_STATS_DATA?.RESULT?.ERROR_MSG}`
    );
  }
  ...
  if (
    ...
    console.error(
      `⚠ カテゴリコード「${code}」のデータが取得できませんでした。` +
        `このコードが正しいか確認してください。` +
        `(レスポンス概要: ${JSON.stringify(json?.GET_STATS_DATA?.RESULT ?? json).slice(0, 300)})`
    );
  }
...
async function run() {
  ...
  const marriages = await fetchStats(
    POP_STATS,
    "A9101",
    "cat"
  );

  const divorces = await fetchStats(
    POP_STATS,
    "A9201",
    "cat"
  );
  ...
  // (F2201/F2211/F2221)・小中学校数(E2101/E3101)・図書館数(G1401)は、
  // このテーブル(0000020201 = 社会・人口統計体系「A 人口・世帯」)には
  // 含まれていないことが判明(STATUS=1「該当データはありません」)。
  // これらはF(労働)・I(健康・医療)・E(教育)・G(文化)という別カテゴリの
  // 統計表に属しており、別途statsDataIdを調査する必要があるため、
  // 一旦取得対象から外している。
  ...
  for (const r of safeValues(pop, "pop")) {
  ...
  for (const r of safeValues(child, "child")) {
  ...
  for (const r of safeValues(old, "old")) {
  ...
  for (const r of safeValues(inMigrants, "inMigrants")) {
  ...
  for (const r of safeValues(outMigrants, "outMigrants")) {
  ...
  for (const r of safeValues(households, "households")) {
  ...
  for (const r of safeValues(singleHouseholds, "singleHouseholds")) {
  ...
  for (const r of safeValues(marriages, "marriages")) {
    const city = ensure(String(r["@area"]));
    city.marriages = Number(r["$"]);
  }

  for (const r of safeValues(divorces, "divorces")) {
    const city = ensure(String(r["@area"]));
    city.divorces = Number(r["$"]);
  }
  ...
  const totalMarriages = cities.reduce(
    (s, c) => s + c.marriages,
    0
  );

  const totalDivorces = cities.reduce(
    (s, c) => s + c.divorces,
    0
  );

  console.log(
    `婚姻件数(全国計): ${totalMarriages.toLocaleString()}件 (目安: 40万〜60万件)`
  );

  console.log(
    `離婚件数(全国計): ${totalDivorces.toLocaleString()}件 (目安: 15万〜20万件)`
  );
